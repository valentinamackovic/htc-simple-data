...
;; resava problem ali BAD BAD jer opet svaki put idemo i levo i desno, treba provera
;; DA LI JE KEY KOJI TRAZIMO VECI ILI MANJI OD TRENUTNO , na osnovu toga odlucimo na koju cemo stranu, performanse :rocket:

#;
(define (lookup t k)
  (cond [(false? t) false]
        [(= (node-key t) k) (node-val t)]
        [(not (boolean? (lookup-key (node-l t) k))) (lookup-key (node-l t) k)]
        [(not (boolean? (lookup-key (node-r t) k))) (lookup-key (node-r t) k)]
        [else false]))
